...
def run_single(input_path_img):
    '''
        ...
        mobile: {'min-grad':4, 'ffl-block':5, 'min-ele-area':50, 'max-aword-inline-gap':6, 'max-line-gap':1}
    ...
    output_root = 'data/example_output'

    resized_height = resize_height_by_longest_edge(input_path_img, resize_length=1200)
    color_tips()
    ...
    if is_ocr:
        ...
        xx=text.text_detection(input_path_img, output_root, show=True, method='google')
    ...
    if is_merge:
        ...
        zz,tt=merge.merge(input_path_img, compo_path, ocr_path, pjoin(output_root, 'merge'),
                    is_remove_bar=key_params['remove-bar'], is_paragraph=key_params['merge-line-to-paragraph'], show=True)
input_path_img="data/input/10012.jpg"
run_single(input_path_img)
